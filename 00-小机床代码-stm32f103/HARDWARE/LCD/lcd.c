...
{  							  
	...
	u16 y0=y;
	uint8_t MyIndex = num;
	u16 csize = ((uint16_t)size/8+((size%8)?1:0))*((uint16_t)size/2);		//得到字体一个字符对应点阵集所占的字节数	
	...
	{   
		...
		else if(size==32)temp = MyFont[num + ' ' - '0'][t];
		else if(size==64){
			switch(MyIndex) {
				case '-':num = 10;temp = NumFont64[num][t];break;
				case ':':num = 11;temp = NumFont64[num][t];break;
				case ' ':temp = 0x00;break;
				//add new character here
				default: num = MyIndex;temp = NumFont64[num][t];	
			}
...
void LCD_ShowSingleColorPic(u16 sx,u16 sy,u16 ex,u16 ey,char *p,u8 Color)
{
...
void LCD_ShowWord(uint8_t WordsLibIndex, uint8_t OrderIndex, u16 x,u16 y,u8 size,u8 mode)
{  							
  u8 temp,t1;
	u16 t;
	u16 y0=y;
	u16 csize = ((uint16_t)size/8+((size%8)?1:0))*((uint16_t)size);		//得到字体一个字符对应点阵集所占的字节数
	
	for(t=0;t<csize;t++)
	{   
		if(WordsLibIndex == 0) temp = Weiyi[OrderIndex][t];
		else if(WordsLibIndex == 1) temp = ziku24[OrderIndex][t];
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
